...
export function AuthProvider({ children }) {
  ...
  const signup = async (email, password, username) => {
    try {
      ...
      const res = await axios.post('https://react-ecommerce-main.onrender.com/api/auth/register', { email, password, username });
      console.log('Signup response:', res.data);
      const { token } = res.data;
      localStorage.setItem('token', token);
      axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
      const profileRes = await axios.get('https://react-ecommerce-main.onrender.com/api/users/profile');
      ...
      const avatarUrl = fetchedUser.avatar
        ? fetchedUser.avatar.startsWith('http')
          ? fetchedUser.avatar
          : `https://react-ecommerce-main.onrender.com${fetchedUser.avatar}`
        : '';
  ...
  const login = async (email, password) => {
    try {
      ...
      const res = await axios.post('https://react-ecommerce-main.onrender.com/api/auth/login', { email, password });
    ...
    setUser((prevUser) => {
      const avatarUrl = updatedUser.avatar
        ? updatedUser.avatar.startsWith('http')
          ? updatedUser.avatar
          : `https://react-ecommerce-main.onrender.com${updatedUser.avatar}`
        : prevUser?.avatar || '';
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      const fetchUser = async () => {
        try {
          const res = await axios.get('https://react-ecommerce-main.onrender.com/api/users/profile');
