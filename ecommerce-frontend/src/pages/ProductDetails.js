...
function ProductDetails() {
  ...
  const fetchWishlist = async () => {
    ...
    try {
      ...
      const res = await axios.get(`https://react-ecommerce-main.onrender.com/api/wishlist/user/${user._id}`, {
  ...
  const fetchReviews = async () => {
    try {
      ...
      setReviewsError('');
      const res = await axios.get(`https://react-ecommerce-main.onrender.com/api/reviews/product/${id}`);
  ...
  const handleWishlistToggle = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      if (isInWishlist) {
        const wishlistItem = await axios.get(`https://react-ecommerce-main.onrender.com/api/wishlist/user/${user._id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const itemToRemove = wishlistItem.data.find((item) => item.productId?._id === id);
        if (itemToRemove) {
          await axios.delete(`https://react-ecommerce-main.onrender.com/api/wishlist/${itemToRemove._id}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
        ...
        await axios.post(
          'https://react-ecommerce-main.onrender.com/api/wishlist',
