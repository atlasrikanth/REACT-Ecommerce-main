...
function ProductList() {
      ...
      if (user && user._id && products.length > 0) {
        try {
          ...
          for (const product of products) {
            const res = await axios.get(`https://react-ecommerce-main.onrender.com/api/reviews/product/${product._id}`);
  ...
  const handleRemoveFromWishlist = async (productId) => {
    try {
      ...
      await axios.delete(`https://react-ecommerce-main.onrender.com/api/wishlist/${wishlistItem._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
